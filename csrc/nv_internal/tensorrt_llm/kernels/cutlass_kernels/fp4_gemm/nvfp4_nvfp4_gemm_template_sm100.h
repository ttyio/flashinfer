...
size_t genericFp4GemmKernelLauncher(void* D, void const* A, void const* B, void const* input_sf,
                                    ...
                                    int k, int batch_count, tkc::CutlassGemmConfig gemmConfig,
...
#define INSTANTIATE_FP4_GEMM_KERNEL_LAUNCHER(T, CTA_M_, CTA_N_, CTA_K_, CGA_M_, CGA_N_, CGA_K_, \
  ...
  genericFp4GemmKernelLauncher<T, cute::Int<CTA_M_>, cute::Int<CTA_N_>, cute::Int<CTA_K_>,      \
      ...
      tkc::CutlassGemmConfig gemmConfig, char* workspace, const size_t workspaceBytes,          \
      cudaStream_t stream, int* occupancy) {                                                    \
    throw std::runtime_error(                                                                   \
        "[TensorRT-LLM Error][FP4 gemm Runner] TensorRT-LLM is not compiled with support for "  \
        "this Architecture.");                                                                  \
  }
  ...
  genericFp4GemmKernelLauncher<T, cute::Int<CTA_M_>, cute::Int<CTA_N_>, cute::Int<CTA_K_>,                   \
    ...
    if (smem_size > mMaxSmemSize) {                                                                          \
      std::string errMsg = "SMEM size exceeds maximum allowed. Required " +                                  \
                           std::to_string(smem_size) + ", got " + std::to_string(mMaxSmemSize);              \
