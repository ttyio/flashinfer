...
CutlassFp4GemmRunner<T, fp4GemmType>::CutlassFp4GemmRunner() {
  ...
  mSm = tk::getSMVersion();
  tk::check_cuda_error(
      cudaDeviceGetAttribute(&mMultiProcessorCount, cudaDevAttrMultiProcessorCount, device));
}
...
size_t CutlassFp4GemmRunner<T, fp4GemmType>::dispatchToArch(
  ...
  if constexpr (fp4GemmType == FP4GemmType::W4A4_NVFP4_NVFP4) {
    if (mSm == 100) {
      return dispatchNVFP4xNVFP4GemmCTAShapeSm100<T>(D, A, B, input_sf, weight_sf, global_sf, m, n,
                                                     k, batch_count, gemmConfig, workspace,
                                                     workspaceBytes, stream, occupancy);
    } else {
      throw std::runtime_error(
          "[TensorRT-LLM Error][CutlassFp4GemmRunner][GEMM Dispatch] Arch unsupported for CUTLASS "
          "FP4 GEMM");
    }
  } else {
    throw std::runtime_error(
...
std::vector<tkc::CutlassGemmConfig> CutlassFp4GemmRunner<T, fp4GemmType>::getConfigs() const {
  ...
  if (mSm == 100) {
    ...
    std::vector<tkc::ClusterShape> clusterShapes = {
        tkc::ClusterShape::ClusterShape_1x1x1, tkc::ClusterShape::ClusterShape_1x2x1,
        tkc::ClusterShape::ClusterShape_2x1x1, tkc::ClusterShape::ClusterShape_2x2x1,
        tkc::ClusterShape::ClusterShape_1x4x1, tkc::ClusterShape::ClusterShape_4x2x1,
        tkc::ClusterShape::ClusterShape_2x4x1, tkc::ClusterShape::ClusterShape_4x4x1,
    };
    for (auto const& tile_config : tilesSm100) {
      for (auto const& cluster_config : clusterShapes) {
        CutlassGemmConfig config(tile_config, tkc::MainloopScheduleType::AUTO,
                                 tkc::EpilogueScheduleType::AUTO, cluster_config);
